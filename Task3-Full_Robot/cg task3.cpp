...
const char* modelname = "data/sofa2.obj";
GLMmodel* pmodel1;
const char* modelname2 = "data/desk.obj";
GLMmodel* pmodel2;
const char* modelname3 = "data/led_tv.obj";
GLMmodel* pmodel3;
const char* modelname4 = "data/chair-ottoman.obj";
GLMmodel* pmodel4;
...
static float ytable = -4.5;
static float ztable = 12.0;

//table position
static float xtv = 10.0;
static float ytv = 0;
static float ztv = -2.0;

//shoes position
static float xblack_sofa = -7.0;
static float yblack_sofa = -3.5;
static float zblack_sofa = -8.0;
...
void setPoses(int frame) {
    if (ztv != -40 && xBody >= 7) {
        ...
        ztv = BallPoses[frame][2];
        ytv = BallPoses[frame][3];
        tvy = BallPoses[frame][4];
        tvx = BallPoses[frame][5];
    }
    else {
        body = 0;
        //shoulderz=45;
...
void display(void)
{
    ...
    glClearColor(0.652, 0.745, 0.707, 1.0);
    ...
    glPushMatrix();
    glRotatef(90, 0.0, 1.0, 0.0);
    glTranslatef(xblack_sofa, yblack_sofa, zblack_sofa);
    glScalef(25.0, 25.0, 25.0);
    glColor3b(5, 5, 5);
    drawmodel((char*)modelname4, pmodel4);
    glPopMatrix();
    glDisable(GL_TEXTURE_2D);
    ...
    glPushMatrix();
    glTranslatef(xtv, ytv, ztv);
    glRotatef((GLfloat)tvy, 0.0, 1.0, 0.0);
    glRotatef((GLfloat)tvx, 1.0, 0.0, 0.0);
    glTranslatef(0, 0, 0);
...
int main(int argc, char** argv)
{
    ...
    glutAddMenuEntry("beige ceramic", 1);
    glutAddMenuEntry("brown ceramic", 2);
    glutAddMenuEntry("stone floor", 3);
